...
import { TenantPlan, PartnerBusinessType } from "../../models/enums/system";
...
async function demonstrateTenantService() {
  ...
  try {
    ...
    const userMemberships = await tenantService.getUserMemberships(
      "recruiter-456"
    );
    console.log(
      "✅ User memberships:",
    ...
    const tenantClient = await tenantService.getTenantClient(
      newTenant.tenantId
    );
    console.log(
      "✅ Tenant client initialized for database:",
    ...
    const partner = await tenantService.createPartner({
      ...
      businessType: PartnerBusinessType.Reseller,
      contactInfo: {
...
async function demonstrateDataIsolation() {
  ...
  try {
    ...
    console.log(
      "📊 Data isolation: Each tenant has their own RavenDB database"
    );
    console.log(
      "🔒 HIPAA compliance: Data is physically separated between tenants"
    );
...
export { demonstrateTenantService, demonstrateDataIsolation };

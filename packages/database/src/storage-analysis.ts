...
function generateTenant(tenantId: string): TenantDocument {
  return {
    ...
    plan: faker.helpers.arrayElement(['free', 'standard', 'enterprise']),
    status: 'active',
...
function generateCompany(tenantId: string): CompanyDocument {
  ...
  return {
    ...
    stage: faker.helpers.arrayElement(['seed', 'early', 'growth', 'mature', 'declining']),
    businessModel: faker.helpers.arrayElement(['b2b', 'b2c', 'b2b2c', 'marketplace', 'saas', 'other']),
    headquarters: {
    ...
    remotePolicy: faker.helpers.arrayElement(['fully_remote', 'hybrid', 'onsite', 'flexible']),
    locations: [{
      type: 'office',
      address: faker.location.streetAddress(),
      city: faker.location.city(),
    ...
    culture: {
      ...
      workEnvironment: faker.helpers.arrayElement(['casual', 'business', 'creative', 'technical']),
    },
    recruitment: {
...
function generateJob(tenantId: string, companyId: string): JobDocument {
  ...
  return {
    ...
    description,
    summary,
    level: faker.helpers.arrayElement(['intern', 'entry', 'junior', 'mid', 'senior', 'lead', 'manager', 'director', 'vp', 'c_level']),
    type: faker.helpers.arrayElement(['full_time', 'part_time', 'contract', 'temporary', 'internship', 'fellowship']),
    workEnvironment: faker.helpers.arrayElement(['remote', 'onsite', 'hybrid', 'flexible']),
...
function generateCandidate(tenantId: string): CandidateDocument {
  ...
  // Add optional portfolio attachment
...
async function createTestDatabase() {
  try {
    ...
    const databaseCreationOperation = {
      DatabaseName: CONFIG.databaseName,
      Settings: {
        DataDirectory: `RavenData/${CONFIG.databaseName}`
      }
    };

    await store.maintenance.server.send(databaseCreationOperation);
...
async function getDatabaseStats() {
  try {
    ...
    return {
      databaseName: CONFIG.databaseName,
      totalCountOfDocuments: stats.CountOfDocuments,
      totalCountOfIndexes: stats.CountOfIndexes,
      totalCountOfAttachments: stats.CountOfAttachments,
      sizeOnDisk: await getDatabaseSize(),
...
async function runStorageAnalysis() {
  ...
  try {
    ...
    const smallPerCandidate = results.find(r => r.size === 'small')?.storage.growth! / results.find(r => r.size === 'small')!.documents.candidates / 1024;
    console.log(`Small Tenant (50 candidates): ${(smallPerCandidate * 50 / 1024).toFixed(2)} MB`);

    // Medium tenant (500 candidates)
    const mediumPerCandidate = results.find(r => r.size === 'medium')?.storage.growth! / results.find(r => r.size === 'medium')!.documents.candidates / 1024;
    console.log(`Medium Tenant (500 candidates): ${(mediumPerCandidate * 500 / 1024).toFixed(2)} MB`);

    // Large tenant (5000 candidates)
    const largePerCandidate = results.find(r => r.size === 'large')?.storage.growth! / results.find(r => r.size === 'large')!.documents.candidates / 1024;
    console.log(`Large Tenant (5000 candidates): ${(largePerCandidate * 5000 / 1024).toFixed(2)} MB`);

    // Calculate monthly growth (assuming 10% candidate growth and 3 evaluations per candidate per month)
    console.log('\n📅 MONTHLY GROWTH ESTIMATES:');
    console.log('-'.repeat(30));
    console.log(`Storage per new candidate: ${(mediumPerCandidate / 1024).toFixed(2)} MB`);
    console.log(`Storage per evaluation: ${(results.find(r => r.size === 'medium')?.storage.growth! / results.find(r => r.size === 'medium')!.documents.evaluations / 1024).toFixed(2)} KB`);
    console.log(`Estimated monthly growth (500 candidates + 1500 evaluations): ${((mediumPerCandidate * 0.1 * 500 + (results.find(r => r.size === 'medium')?.storage.growth! / results.find(r => r.size === 'medium')!.documents.evaluations / 1024) * 1500) / 1024).toFixed(2)} MB`);

    // Cost analysis
    console.log('\n💰 COST ANALYSIS:');
    console.log('-'.repeat(30));
    console.log(`AWS S3 Standard Storage (First 50TB): $0.023 per GB`);
    console.log(`Monthly storage cost per 500 candidates: $${(mediumPerCandidate * 500 / 1024 / 1024 * 0.023).toFixed(2)}`);
    console.log(`Annual storage cost per 500 candidates: $${(mediumPerCandidate * 500 / 1024 / 1024 * 0.023 * 12).toFixed(2)}`);
    ...
    const reportData = {
      ...
      results,
      projections: {
        storagePerCandidate: mediumPerCandidate / 1024, // KB
        storagePerEvaluation: results.find(r => r.size === 'medium')?.storage.growth! / results.find(r => r.size === 'medium')!.documents.evaluations / 1024, // KB
        monthlyGrowthPerActiveCandidate: (mediumPerCandidate * 0.1 + (results.find(r => r.size === 'medium')?.storage.growth! / results.find(r => r.size === 'medium')!.documents.evaluations / 1024) * 3) / 1024 // MB
      },
      costs: {
        storageCostPerGB: 0.023,
        monthlyCostPer500Candidates: (mediumPerCandidate * 500 / 1024 / 1024 * 0.023),
        annualCostPer500Candidates: (mediumPerCandidate * 500 / 1024 / 1024 * 0.023 * 12)
      }
    };

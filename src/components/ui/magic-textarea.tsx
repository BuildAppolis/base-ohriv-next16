...
interface ProcessingTextareaProps extends React.TextareaHTMLAttributes<HTMLTextAreaElement> {
  isProcessing?: boolean;
  processingType?: 'default' | 'analysis' | 'generation' | 'enhancement';
}

export function ProcessingTextarea({
  className,
  isProcessing = false,
  processingType = 'default',
  disabled,
  ...props
}: ProcessingTextareaProps) {
          ...
          <div className="absolute bottom-8 left-4 right-4 pointer-events-none">
            <div className="flex justify-center space-x-1">
              {[0, 1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="h-1 w-8 bg-ring/60 rounded-full animate-pulse"
                  style={{

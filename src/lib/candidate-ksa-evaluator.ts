...
export async function evaluateCandidateKSA(
  ...
  const overallScore = Math.round((knowledgeScore + skillsScore + abilityScore) / 3 * 10) / 10

  // Generate recommendation based on score
  const recommendation = overallScore >= 8 ? 'strong-recommend' :
                        overallScore >= 6 ? 'recommend' :
                        overallScore >= 4 ? 'consider' : 'reject'

  const jobTitle = jobContext || (ksaFramework.KSA_JobFit && Object.keys(ksaFramework.KSA_JobFit)[0]) || 'Unknown Position'
  ...
  return {
    ...
    scores: {
      knowledge: {
        ...
        confidence: 0.8,
        notes: `Knowledge assessment completed with score ${knowledgeScore.toFixed(1)}/10`
      },
      skills: {
        overall: skillsScore,
        breakdown: {
        ...
        confidence: 0.8,
        notes: `Skills assessment completed with score ${skillsScore.toFixed(1)}/10`
      },
      abilities: {
        ...
        confidence: 0.8,
        notes: `Abilities assessment completed with score ${abilityScore.toFixed(1)}/10`
      }
    },
    overallCompatibility: {
      score: overallScore,
      recommendation,
      strengths: overallScore >= 6 ? [
        'Good overall compatibility with role requirements',
        'Strong potential for growth and development'
      ] : [],
      concerns: overallScore < 6 ? [
